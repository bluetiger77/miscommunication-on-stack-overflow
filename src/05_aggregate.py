...
def aggregate_tags(con: duckdb.DuckDBPyConnection):
    """Track A — community tags (multi-label). Overall miscommunication rate."""
    df = con.execute(f"""
        SELECT
            t.tag                        AS topic,
    ...
    print(f"Track A: {len(df)} tags → {out}")
    print(df.head(10).to_string(index=False))
    return df

def aggregate_tags_by_category(con: duckdb.DuckDBPyConnection, cats: list[str]):
    """Track A — per-category miscommunication rates, long format.
    ...
    print(f"Track A by category: {len(combined)} rows → {out}")
    return combined
...
def global_summary(con: duckdb.DuckDBPyConnection, cats: list[str]):
    ...
    with open(os.path.join(RESULTS_DIR, "global_summary.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["total_questions", "total_miscomm", "global_miscomm_rate"])
        w.writerow([total, flagged, rate])
